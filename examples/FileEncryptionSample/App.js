...
import { encryptSignAndUploadImage } from './encrypt-sign-and-upload';
import { downloadVerifyAndDecryptImage } from './download-verify-and-decrypt';
...
export default class App extends Component {
  ...
  handleUpload = () => {
    this.reset();
    this.pickImage()
    .then(image => encryptSignAndUploadImage({ image, keypair, url: `${apiUrl}/upload` }))
    .then(result => {
      if (result) {
    ...
    if (!downloadedImage.isSigantureVerified) {
      return (
        <Text style={styles.error}>{
          'The downloaded image cannot be displayed because its digital signature failed verification'
        }</Text>
      );
    }
  ...
  renderObject(p) {
    ...
    return (
      <View>
        {text.map((t, i) => (<Text key={i} style={styles.json}>{t}</Text>))}
      </View>
    );
  }
      ...
      <View style={styles.container}>
        <ScrollView>
          <View style={styles.section}>
            <Button style={styles.button} title="Pick an image to upload" onPress={this.handleUpload} />
            {this.renderUploadedImage()}
          </View>
          <View style={styles.section}>
            {this.state.imageIdToDownload && <Button style={styles.button} title={`Download and display`} onPress={this.handleDownload}/>}
