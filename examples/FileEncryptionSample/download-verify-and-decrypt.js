...
export function downloadVerifyAndDecryptImage({ url, keypair, extension }) {
...
function downloadImage(url, extension) {
  ...
  .then(res => {
    ...
    return res.text()
    .then(errorMessage => Promise.reject(
      new Error(`The server responded with status code ${status}; ${errorMessage}`
    )));
  });
}

function verifyImageSignature(image, keypair) {
...
function decryptImage(image, keypair) {
  ...
  .then(decryptedFilePath => ({ ...image, uri: 'file://' + decryptedFilePath }));
};

function cleanupAfterDownload(image) {
  // remove temporary file where we stored the downloaded ecnrypted image
  const { downloadedFilePath, ...rest } = image;
  return RNFetchBlob.fs.unlink(downloadedFilePath)
  .catch(err => {
    console.log(`failed to remove temporary download file at ${downloadedFilePath}`, err);
  })
  .then(() => rest);
}