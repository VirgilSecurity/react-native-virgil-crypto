...
export function encryptSignAndUploadImage({ image, url, keypair }) {
  if (image == null) {
...
function encryptImage(image, keypair) {
  ...
  .then(encryptedFilePath => ({ ...image, encryptedFilePath }));
}

function signImage(image, keypair) {
  ...
  .then(res => {
    ...
    return res.text()
      .then(errorMessage => Promise.reject(
        new Error(`The server responded with status code ${res.status}; ${errorMessage}`
      )));
  })
  .then(id => ({ ...image, id }));
}

function cleanupAfterUpload(image) {
  // remove the temporary file where we stored encrypted image
  const { encryptedFilePath, ...rest } = image;
  return RNFetchBlob.fs.unlink(encryptedFilePath)
  .catch(err => {
    console.log(`failed to remove temporary file at ${encryptedFilePath}`, err);
  })
  .then(() => rest);
}